...
try:
    ...
    #Same as cut-.py
    ReadingData[WORDID_COL] = ReadingData['Format'].astype(str) + '-' + ReadingData['SENTENCE_OR_PASSAGE_NUMBER'].astype(str) + '-' + ReadingData['IP_INDEX'].astype(str) + '-' + ReadingData['IA_ID'].astype(str) #Same as word_id in cut-.py
...
try:
    ReadingData_full['IA_FIRST_FIXATION_DURATION'] = pd.to_numeric(ReadingData_full['IA_FIRST_FIXATION_DURATION'], errors='coerce')
    ReadingData_full['IA_FIRST_RUN_DWELL_TIME'] = pd.to_numeric(ReadingData_full['IA_FIRST_RUN_DWELL_TIME'], errors='coerce')
    # 原始数据是数值
    coerced_nan_count = ReadingData_full[['IA_FIRST_FIXATION_DURATION']].isnull().sum().sum()
    if coerced_nan_count > 0:
        print(f"警告: 在转换时间列为数值时，有 {coerced_nan_count} 个值因跳读问题被强制转换为NaN（原本就是NAN）。")
    
    # 确保条件列是字符串类型，以便进行比较
    # 使用 .astype(str) 可以安全地处理可能存在的非字符串值
    condition_series = ReadingData_full['IA_FIRST_FIX_PROGRESSIVE'].astype(str)
    ...
    ReadingData_full['Gopast'] = np.where(
        condition_series == '1.0',                      # 条件
        ReadingData_full['IA_SELECTIVE_REGRESSION_PATH_DURATION'], # 如果为真，取此值
        np.nan                                       # 如果为假，设为NaN
    )

    ReadingData_full['Gopast1'] = ReadingData_full['IA_SELECTIVE_REGRESSION_PATH_DURATION']
    
    
    print("FFD, GD, SFD, TT, Gopast计算完成。")
