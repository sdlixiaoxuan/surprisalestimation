...
# 假设文本信息文件是一个CSV文件
TEXT_INFO_FILE_PATH = 'outputs/EMTEC-EnglishMaterial.csv'
...
# Reading Data Path
READING_DATA_PATH = 'reading_measures_corrected.csv'
READING_DATA_OUTPUT_PATH = 'outputs/merge/DATA_EMTEC.csv'
...
try:
    ...
    #surprisal_df['averaged_prob'] = surprisal_df.iloc[:,3:].mean(axis=1)
...
try:
    ...
    ReadingData = pd.read_csv(READING_DATA_PATH, sep = '\t')

    ReadingData.rename(columns={'word':'word_'
                                }, inplace=True)

    # 为了与材料文件合并，确保key的列名一致
    #Same as cut-.py
    ReadingData[WORDID_COL] = ReadingData['item_id'].astype(str) + '-' + ReadingData['model'].astype(str) + '-' + ReadingData['decoding_strategy'].astype(str) + '-' + ReadingData['TRIAL_ID'].astype(str) + '-' + ReadingData['word_id'].astype(str)
...
# ===============================================================
# 8. 计算关键眼动指标 (FFD, GD; Zhang et al. (2022) already calculated)
# ===============================================================
print("正在计算 FFD (First Fixation Duration) 和 GD (Gaze Duration)...")

try:
    # 计算 FFD
    ReadingData_full['FFD'] = ReadingData_full['FFD']
    ReadingData_full['GD'] = ReadingData_full['FPRT']
    ReadingData_full['SFD'] = ReadingData_full['SFD']
    ReadingData_full['TT'] = ReadingData_full['TFT']
    ReadingData_full['Gopast'] = ReadingData_full['RRT']
