...
# 假设文本信息文件是一个CSV文件
TEXT_INFO_FILE_PATH = 'outputs/ZHANG22-ChineseMaterial.csv'
...
READING_DATA_OUTPUT_PATH = 'outputs/merge/DATA_ZHANG22.csv'
...
try:
    ...
    #Same as cut-.py
    ReadingData[WORDID_COL] = ReadingData['Experiment'].astype(str) + '-' + ReadingData['Sentence_ID'].astype(str) + '-' + ReadingData['ROI'].astype(str) #Same as word_id in cut-.py
...
try:
    ...
    ReadingData_full['GD'] = ReadingData_full['GD']
    ReadingData_full['SFD'] = np.where(
        ReadingData_full['FFD'] == ReadingData_full['GD'],                      # 条件
        ReadingData_full['FFD'], # 如果为真，取此值
        np.nan                                       # 如果为假，设为NaN
    )
    ReadingData_full['TT'] = ReadingData_full['TT']

    print("FFD, GD, SFD, TT 计算完成。")
