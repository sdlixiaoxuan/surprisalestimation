...
# 眼动数据文件中的列名
EYE_DATA_CONDITION_COL = 'WORD_FIRST_FIX_PROGRESSIVE'
EYE_DATA_SOURCE_FFD = 'WORD_FIRST_FIXATION_DURATION'
...
try:
    ...
    ReadingData_full['WORD_FIRST_RUN_START_TIME'] = pd.to_numeric(ReadingData_full['WORD_FIRST_RUN_START_TIME'], errors='coerce')
    ReadingData_full['WORD_FIRST_RUN_END_TIME'] = pd.to_numeric(ReadingData_full['WORD_FIRST_RUN_END_TIME'], errors='coerce')
    
    # 检查转换后是否有值变成了NaN，这表示原始数据有问题
    coerced_nan_count = ReadingData_full[['WORD_FIRST_RUN_START_TIME', 'WORD_FIRST_RUN_END_TIME']].isnull().sum().sum()
    ...
    ReadingData_full['FFD'] = np.where(
        condition_series == '1',                      # 条件
        ReadingData_full[EYE_DATA_SOURCE_FFD],       # 如果为真，取此列的值
        np.nan                                       # 如果为假，设为NaN
    )
    ...
    ReadingData_full['GD'] = np.where(
        condition_series == '1',                      # 条件
        ReadingData_full['WORD_FIRST_RUN_END_TIME'] - ReadingData_full['WORD_FIRST_RUN_START_TIME'], # 如果为真，取此值
        np.nan                                       # 如果为假，设为NaN
    )
    ...
    ReadingData_full['TT'] = ReadingData_full['WORD_TOTAL_READING_TIME']
    ReadingData_full['Gopast'] = ReadingData_full['WORD_GO_PAST_TIME']
